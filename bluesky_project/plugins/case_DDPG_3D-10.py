...
from plugins.Multi_Agent.DDPG_3DSeven import DDPG
...
def init_plugin():
    ...
    os.makedirs('output/DDPG/DDPG3D-10/scenarios', exist_ok=True)
    ...
    agent_manager = DDPG(state_dim=14, intruders_dim=18, hidden_dim=256, action_dim=3,
                         max_agents=max_ac)  # [修改] state_dim: 12->14, hidden_dim: 128->256
    ...
    if written == 1:
        SCN_File = f"output/DDPG/DDPG3D-10/scenarios/{episode_num}.scn"
...
def reset():
    ...
    stats_dir = 'output/DDPG/DDPG3D-10'
    ...
    reward_list = [0 for _ in range(max_ac)]
    if episode_num % 10 == 0:
        agent_manager.save_models(f"output\\DDPG\\DDPG3D-10\\DDPG")
        ...
        for z in range(min(len(sorted_list), num_intruders)):
            ...
            min_distances[id_i].append(
                [
                    ...
                    lat[air_index],
                    lon[air_index],
                    traf.cas[air_index] * 1.9439,
                    traf.alt[air_index] * 3.28084,
                    traf.trk[air_index]
                ])
