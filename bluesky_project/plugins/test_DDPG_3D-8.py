...
def init_plugin():
    ...
    written = 0  # [测试模式] 不记录场景文件

    os.makedirs('output/DDPG/DDPG3D-14', exist_ok=True)  # 测试结果输出目录
    ...
    # [测试模式] 加载预训练模型
    model_path = "output/DDPG/DDPG3D-11/DDPG"
    try:
    ...
    if written == 1:
        SCN_File = f"output/DDPG/DDPG3D-14/scenarios/{episode_num}.scn"
...
def reset():
    ...
    stats_dir = 'output/DDPG/DDPG3D-14'
