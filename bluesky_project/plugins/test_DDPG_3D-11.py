...
from plugins.Multi_Agent.DDPG_3DTen import DDPG  # [Change] Import from DDPG_3DTen
...
def init_plugin():
    ...
    written = 1  # [测试模式] 不记录场景文件

    os.makedirs('output/DDPG/DDPG3D-15_Test', exist_ok=True)  # [Change] 测试结果输出目录
    ...
    # [测试模式] 加载预训练模型
    model_path = "output/DDPG/DDPG3D-15/DDPG" # [Change] Model path
    try:
        agent_manager.load_models(model_path)
        print("="*50)
        print("🔍 测试模式: 成功加载预训练模型")
        print(f"   模型路径: {model_path}")
        print("="*50)
    except Exception as e:
        print("="*50)
        print(f"❌ 模型加载失败: {e}")
        print(f"   请确认模型文件存在: {model_path}_actor.pth")
        print("="*50)
    ...
    if written == 1:
        SCN_File = f"output/DDPG/DDPG3D-15_Test/scenarios/{episode_num}.scn" # [Change]
    ...
    config = {
        'plugin_name': 'test_DDPG_3D-11', # [Change]
        'plugin_type': 'sim',
        'update_interval': 5.0,
        'update': update,
    }
...
def update():
    ...
    for i, air_craft in enumerate(traf.id):
        ...
        raw_action = agent_manager.take_action(state, -1)  # [测试模式] 禁用探索噪声
...
def reset():
    ...
    stats_dir = 'output/DDPG/DDPG3D-15_Test'
    ...
    # [测试模式] 禁用模型保存
    # if episode_num % 10 == 0:
    #     agent_manager.save_models(f"output/DDPG/DDPG3D-8/DDPG")
    ...
    # [测试模式] 禁用场景记录
    # if written == 1:
    #     SCN_File = f"output/DDPG/DDPG3D-8/scenarios/{episode_num}.scn"
    #     if os.path.exists('multi_agent.scn'):
    #         shutil.copy2('multi_agent.scn', SCN_File)
    #     else:
    #         open(SCN_File, 'w').close()
