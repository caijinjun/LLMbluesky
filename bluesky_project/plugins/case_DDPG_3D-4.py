...
from plugins.Multi_Agent.DDPG_3DFour import DDPG
...
def reset():
    ...
    stats_dir = 'output/DDPG/DDPG3D-4'
    ...
    reward_list = [0 for _ in range(max_ac)]
    if episode_num % 10 == 0:
        agent_manager.save_models(f"output\\DDPG\\DDPG3D-4\\DDPG")
...
def get_rewards(stats):
    ...
    w_collision = -40.0
    w_arrival = 30.0
    w_progress = 0.1  # 提高进度奖励权重
    w_heading = 0.005
    w_timeout = -10.0   # 降低超时惩罚
