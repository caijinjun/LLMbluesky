...
def _text_bytes(text, encoding='ascii'):
    if isinstance(text, bytes):
        return np.frombuffer(text, dtype=np.uint8)
    return np.frombuffer(text.encode(encoding, 'ignore'), dtype=np.uint8)
...
class RadarWidget(QGLWidget):
    ...
    def actnodedataChanged(self, nodeid, nodedata, changed_elems):
        ...
        if 'SHAPE' in changed_elems:
            if nodedata.polys:
                ...
                # Only reset if objects are initialized
                if hasattr(self, 'allpolys'):
                    self.allpolys.set_vertex_count(0)
                    self.allpfill.set_vertex_count(0)

        # Trail data change
        if 'TRAILS' in changed_elems:
            if hasattr(self, 'traillines'):  # Only if initialized
        ...
        if 'CUSTWPT' in changed_elems:
            if hasattr(self, 'custwplatbuf'):  # Only if initialized
                if nodedata.custwplbl:
                    update_buffer(self.custwplblbuf, _text_bytes(nodedata.custwplbl))
    ...
    def create_objects(self):
        ...
        self.aptlabels = self.font.prepare_text_instanced(_text_bytes(aptids), (4, 1), self.aptlatbuf, self.aptlonbuf, char_size=text_size, vertex_offset=(apt_size, 0.5 * apt_size))
    ...
    def resizeGL(self, width, height):
        ...
        zoom   = float(max(1, self.width)) / float(width) * pixel_ratio
        origin = (width / 2.0, height / 2.0)

        # Update width, height, and aspect ratio
        self.width, self.height = max(1, int(width / pixel_ratio)), max(1, int(height / pixel_ratio))
        self.ar = float(width) / max(1.0, float(height))
    ...
    def update_aircraft_data(self, data):
        ...
        if self.naircraft == 0:
            ...
            update_buffer(self.aclblbuf, _text_bytes(rawlabel, encoding='utf8'))
    ...
    def pixelCoordsToGLxy(self, x, y):
        ...
        glx = (float(2.0 * x) / width  - 1.0)
        gly = -(float(2.0 * y) / height - 1.0)
        return glx, gly
