...
def load(name):
    ...
    try:
        ...
        # Load the plugin
        # Load the plugin
        file_path = path.join(descr.module_path, descr.module_name + '.py')
        spec = importlib.util.spec_from_file_location(descr.module_name, file_path)
        if spec and spec.loader:
            plugin = importlib.util.module_from_spec(spec)
            sys.modules[descr.module_name] = plugin
            spec.loader.exec_module(plugin)
        else:
            raise ImportError(f"Could not load plugin {name} from {file_path}")
